...
#
#         return：点击数据all_clicks
#     """
#     all_clicks = pd.read_csv(data_path + 'train_click_log.csv')
#     all_user_ids = all_clicks.user_id.unique()
#
#     sample_user_ids = np.random.choice(all_user_ids, size=sample_nums, replace=False)
#     all_clicks = all_clicks[all_clicks['user_id'].isin(sample_user_ids)]
#
#     all_clicks = all_clicks.drop_duplicates((['user_id', 'click_article_id', 'click_timestamp']))
#     return all_clicks

def get_sampling_click_data(data_path, sample_nums=1000,offline=True):
    if offline:
...
#         all_clicks = pd.read_csv(data_path + 'train_click_log.csv')
#     else:
#         train_clicks = pd.read_csv(data_path + 'train_click_log.csv')
#         test_clicks = pd.read_csv(data_path + 'testA_click_log.csv')
#
...
# 采样数据
# all_clicks_df = get_sampling_click_data(data_path)

# 全量训练集
all_clicks_df = get_sampling_click_data(data_path,offline=False)

# 对时间戳进行归一化,用于在关联规则的时候计算权重
all_clicks_df['click_timestamp'] = all_clicks_df[['click_timestamp']].apply(max_min_scaler)
...
def get_hist_and_last_click(all_click):
    ...
    # 如果用户只有一个点击，hist为空，会导致训练集中该用户不可见，特别处理一下
...
train_history_clicks_df, train_last_click_df = get_hist_and_last_click(all_clicks_df)
...
def itemcf_sim(df, item_created_time_dict):
    """
        文章与文章之间的相似性矩阵计算
        df: 数据表
        item_created_time_dict:  文章创建时间的字典{0:0.9784319658749242,1:0.6802953033702287,.......}
        return : 文章与文章的相似性矩阵
    ...
    #{366:[(70986, 0.9589285944580793), (50644, 0.9590981114391585), (36162, 0.959121215209136)]}
    # 计算物品相似度
    i2i_sim = {}
    item_cnt = defaultdict(int)
    for user, item_time_list in tqdm(user_item_time_dict.items()):
        # 在基于商品的协同过滤优化的时候可以考虑时间因素
        for loc1, (i, i_click_time) in enumerate(item_time_list):  #0,(70986,0.9589285944580793)
            item_cnt[i] += 1
...
i2i_sim = itemcf_sim(all_clicks_df, item_created_time_dict)

#基于用户的协同过滤 + 关联规则
def usercf_sim(all_clicks_df, user_activate_degree_dict):
    """
        用户相似性矩阵计算
        all_clicks_df: 数据表
        user_activate_degree_dict: 用户活跃度的字典 {174：0.0078125，316:0.0078125......}
        return 用户相似性矩阵

        思路: 基于用户的协同过滤 + 关联规则
    """
    item_user_time_dict = get_item_user_time_dict(all_clicks_df)
# {1799:[(208054, 0.23678242363302276)],1873:[(12598, 0.8845377135962263)]......}
    u2u_sim = {}
...
def get_user_activate_degree_dict(all_clicks_df):
    all_clicks_df_ = all_clicks_df.groupby('user_id')['click_article_id'].count().reset_index()

    # 用户活跃度归一化
    mm = MinMaxScaler()
    all_clicks_df_['click_article_id'] = mm.fit_transform(all_clicks_df_[['click_article_id']])
    user_activate_degree_dict = dict(zip(all_clicks_df_['user_id'], all_clicks_df_['click_article_id']))

    return user_activate_degree_dict

# usercf计算比较耗费内存，而且需要等待较长时间
# 如果是采样数据的话，运行速度会快很多
user_activate_degree_dict = get_user_activate_degree_dict(all_clicks_df)
u2u_sim = usercf_sim(all_clicks_df, user_activate_degree_dict)
...
def embdding_sim(click_df, item_emb_df, save_path, topk):
    """
        基于内容的文章embedding相似性矩阵计算，参数与返回值如下
        click_df: 数据表
        ...
        思路: 对于每一篇文章， 基于embedding的相似性返回topk个与其最相似的文章，检索相似近邻用了faiss进行加速
    """
    ...
    return item_sim_dict    #每一篇新闻的相似新闻

item_emb_df = pd.read_csv(data_path + '/articles_emb.csv')
emb_i2i_sim = embdding_sim(all_clicks_df, item_emb_df, save_path, topk=10) # topk可以自行设置
...
def metrics_recall(user_recall_items_dict, train_last_click_df, topk=5,name=None):
    ...
    for k in range(10, topk + 1, 10):
        ...
        print('name: ',name,' topk: ', k, ' : ', 'hit_num: ', hit_num, 'hit_rate: ', hit_rate, 'user_num : ', user_num)
                    ...
                    if i != len(pos_list) - 1:
                        ...
                        for negi in range(negsample):
                            train_set.append((reviewerID, hist[::-1], neg_list[i * negsample + negi], 0, len(hist[
                                                                                                             ::-1])))  # 负样本 [user_id, his_item, neg_item, label, len(his_item)]
                    else:
                        # 将最长的那一个序列长度作为测试数据
...
def youtubednn_u2i_dict(data, topk=20):
            ...
            user_feature_columns = [SparseFeat('user_id', feature_max_idx['user_id'], embedding_dim),
                                    VarLenSparseFeat(SparseFeat('hist_article_id', feature_max_idx['click_article_id'],
                                                                embedding_dim,
                                                                embedding_name="click_article_id"), SEQ_LEN, 'mean',
                                                     'hist_len'), ]
            item_feature_columns = [SparseFeat('click_article_id', feature_max_idx['click_article_id'], embedding_dim)]

            # 模型的定义
            # num_sampled: 负采样时的样本数量
            train_counter = collections.Counter(train_model_input['click_article_id'])
            item_count = [train_counter.get(i, 0) for i in range(item_feature_columns[0].vocabulary_size)]
            sampler_config = NegativeSampler('frequency', num_sampled=5, item_name="click_article_id",
            ...
            history = model.fit(train_model_input, train_label, batch_size=256, epochs=1, verbose=1,
            ...
            index = faiss.index_cpu_to_gpu(res2, 0, index)
...
if not metric_recall:
    user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(all_clicks_df.copy(), topk=20)
else:
    train_history_clicks_df, train_last_click_df = get_hist_and_last_click(all_clicks_df)
    user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(train_history_clicks_df, topk=20)
    # 召回效果评估
    metrics_recall(user_multi_recall_dict['youtubednn_recall'], train_last_click_df, topk=20,name='youtubednn_recall')
...
def item_based_recommend(user_id, user_item_time_dict, i2i_sim, sim_item_topk, recall_item_num, item_topk_click,
            ...
            """
            # 获取用户历史交互的文章
            user_hist_items = user_item_time_dict[user_id] #[(236593, 0.05247623955572423), (234359, 0.05249830444730974), (235430, 0.24720306997191868), (108855, 0.2472251348635042), (272242, 0.2730587180222821), (293050, 0.2730807829138676)]
            user_hist_items_ = {user_id for user_id, _ in user_hist_items} #{108855, 234359, 235430, 236593, 272242, 293050}

            item_rank = {}

            for loc, (i, click_time) in enumerate(user_hist_items): #0,(236593, 0.05247623955572423)
                try:
...
if metric_recall:
    ...
    train_history_clicks_df = all_clicks_df

user_recall_items_dict = collections.defaultdict(dict)
user_item_time_dict = get_user_item_time(train_history_clicks_df)
...
sim_item_topk = 20
recall_item_num = 10
item_topk_click = get_item_topk_click(train_history_clicks_df, k=50)

for user in tqdm(train_history_clicks_df['user_id'].unique()):
...
if metric_recall:
    # 召回效果评估
    metrics_recall(user_multi_recall_dict['itemcf_sim_itemcf_recall'], train_last_click_df, topk=recall_item_num,name='itemcf_sim_itemcf_recall')
...
if metric_recall:
    # 召回效果评估
    metrics_recall(user_multi_recall_dict['embedding_sim_item_recall'], train_last_click_df, topk=recall_item_num,name='embedding_sim_item_recall')
...
def user_based_recommend(user_id, user_item_time_dict, u2u_sim, sim_user_topk, recall_item_num,
    ...
    """
        ...
        :param u2u_sim: 字典，用户相似性矩阵      {139:{139:0}, 164:{164:0, 513:0.2391091351278237}......}
        :param sim_user_topk: 整数， 选择与当前用户最相似的前k个用户
        :param recall_item_num: 整数， 最后的召回文章数量
        :param item_topk_click: 列表，点击次数最多的文章列表，用户召回补全
        :param item_created_time_dict: 文章创建时间列表    {0:0.9875454456,1:0.684545454,2:0.67564545}
...
#user_hist_items={50823, 70646, 156560, 160417, 206168, 234189, 235230, 284547}
    items_rank = {}
    for sim_u, wuv in sorted(u2u_sim[user_id].items(), key=lambda x: x[1], reverse=True)[:sim_user_topk]:
        for i, click_time in user_item_time_dict[sim_u]: #取出该用户的相似用户的每一条点击新闻id和点击时间
            ...
            for loc, (j, click_time) in enumerate(user_item_time_list):  #这个是用户点击文章
                # 点击时的相对位置权重
...
if metric_recall:
    # 召回效果评估
    metrics_recall(user_recall_items_dict, train_last_click_df, topk=recall_item_num,name='UserCF')
...
u2u_sim = u2u_embdding_sim(all_clicks_df, user_emb_dict, save_path, topk=10)
...
if metric_recall:
    # 召回效果评估
    metrics_recall(user_multi_recall_dict['youtubednn_usercf_recall'], train_last_click_df, topk=recall_item_num,name='youtubednn_usercf_recall')
...
def get_click_article_ids_set(all_clicks_df):
    return set(all_clicks_df.click_article_id.values)
...
def cold_start_items(user_recall_items_dict, user_hist_item_typs_dict, user_hist_item_words_dict, \
    ...
    """
        冷启动的情况下召回一些文章
        user_recall_items_dict: 基于内容embedding相似性召回来的很多文章， 字典， {user1: [(item1, item2), ..], }
        user_hist_item_typs_dict: 字典， 用户点击的文章的主题映射
        user_hist_item_words_dict: 字典， 用户点击的历史文章的字数映射
        user_last_item_created_time_idct: 字典，用户点击的历史文章创建时间映射
        item_tpye_idct: 字典，文章主题映射
        item_words_dict: 字典，文章字数映射
        item_created_time_dict: 字典， 文章创建时间映射
        click_article_ids_set: 集合，用户点击过得文章, 也就是日志里面出现过的文章
...
all_clicks_df_ = all_clicks_df.copy()
all_clicks_df_ = all_clicks_df_.merge(item_info_df, how='left', on='click_article_id')
user_hist_item_typs_dict, user_hist_item_ids_dict, user_hist_item_words_dict, user_last_item_created_time_dict = get_user_hist_item_info_dict(all_clicks_df_)
click_article_ids_set = get_click_article_ids_set(all_clicks_df)
# 需要注意的是
